...
public:
    bool operator()(const BaseEvent& b) const{
        if (typeid(b) == typeid(OpenEvent)) 
            return b.getName().compare(op.getName()) == 0;
        if (typeid(b) == typeid(ClosedEvent)) 
            return b.getName().compare(cp.getName()) == 0;
        if (typeid(b) == typeid(CustomEvent<Filter2>)) 
            return b.getName().compare(cup.getName()) == 0;
